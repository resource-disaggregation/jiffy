...
class send_benchmark : public msq_queue_benchmark {
 public:
    ...
    for (size_t i = 0; i < num_clients_; ++i) {
      workers_[i] = std::thread([i, this]() {
        ...
        for (j = 0; j < num_ops_; ++j) {
          ...
          clients_[i]->send(std::to_string(j), data_);
    ...
    for (size_t i = 0; i < num_clients_; ++i) {
      workers_[i] = std::thread([i, this]() {
        for (size_t j = 0; j < num_ops_; ++j) {
          clients_[i]->read(std::to_string(j), data_);
        }
        auto bench_begin = time_utils::now_us();
        uint64_t tot_time = 0, t0, t1 = bench_begin;
        size_t j;
        for (j = 0; j < num_ops_; ++j) {
          t0 = time_utils::now_us();
          clients_[i]->read(std::to_string(j));
          t1 = time_utils::now_us();
          tot_time += (t1 - t0);
        }
        latency_[i] = (double) tot_time / (double) j;
...
int main(int argc, char **argv) {
  ...
  try {
    ...
    generic.add_options()
        ...
        ("test", po::value<std::string>(&op_type)->default_value("send"))
  ...
  for (int i = 0; i < num_clients; ++i) {
    mq_clients[i] = client.open_or_create_msq_queue(path, backing_path, num_blocks, chain_length);
  }

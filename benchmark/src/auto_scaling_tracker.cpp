#include "auto_scaling_tracker.h"
#include <jiffy/utils/logger.h>


//TODO add the tracker here
using namespace jiffy::utils;

auto_scaling_tracker::auto_scaling_tracker(std::vector<std::shared_ptr<block>> &blocks,
                                               uint64_t periodicity_ms,
                                               const std::string &output_file)
    : blocks_(blocks), periodicity_ms_(periodicity_ms), output_file_(output_file) {}

auto_scaling_tracker::~auto_scaling_tracker() {
  stop();
}

void auto_scaling_tracker::start() {
  worker_ = std::thread([&] {
...
void auto_scaling_tracker::stop() {
  stop_.store(true);
  if (worker_.joinable())
    worker_.join();
}

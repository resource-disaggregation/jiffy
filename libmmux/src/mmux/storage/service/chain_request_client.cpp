...
void chain_request_client::request(const sequence_id &seq,
                                   const std::vector<int32_t> &cmd_ids,
                                   const std::vector<std::vector<std::string>> &args) {
  client_->chain_request(seq, block_id_, cmd_ids, args);
}

void chain_request_client::run_command(std::vector<std::vector<std::string>> &_returns,
                                       const std::vector<int32_t> &cmd_ids,
                                       const std::vector<std::vector<std::string>> &args) {
  client_->run_command(_returns, block_id_, cmd_ids, args);
}

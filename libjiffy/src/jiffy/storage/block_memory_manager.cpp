...
block_memory_manager::block_memory_manager(size_t capacity, const std::string memory_mode, void* pmem_kind)
...
void *block_memory_manager::mb_malloc(size_t size) {
  if (used_.load() > capacity_) {
    ...
    auto ptr = memkind_malloc((struct memkind*)pmem_kind_, size);
  #else
...
void block_memory_manager::mb_free(void *ptr) {
  #ifdef MEMKIND_IN_USE
    auto size = memkind_malloc_usable_size((struct memkind*)pmem_kind_, ptr);
    memkind_free((struct memkind*)pmem_kind_, ptr);
  #else 

...
file_client::file_client(std::shared_ptr<directory::directory_interface> fs,
  ...
  LOG(log_level::info) << "Last partition set to be " << last_partition_;
}

std::string file_client::read_data_file(const std::size_t size) {
  if (cur_partition_ * block_size_ + cur_offset_ > last_partition_ * block_size_ + last_offset_) {
    return "";
  }
  LOG(log_level::info) << "Read " << size;
  ...
  /**
   * @brief Parallel read here, assuming all the partitions are ready after the above step
   */
  LOG(log_level::info) << "Has more " << has_more << " " << file_size << " " << remain_size << " " << size;
  ...
  while (has_more > 0) {
    ...
    LOG(log_level::info) << "Reading " << cur_offset_ << " " << data_to_read;
...
void file_client::write_data_file(const std::string &data) {
  ...
  if (cur_partition_ * block_size_ + cur_offset_ > file_size) {
    ...
    LOG(log_level::info) << "Here 1 " << num_chain_needed;
    ...
    LOG(log_level::info) << "Here 2 " << num_chain_needed;
    off = 0;
  } else {
  ...
  while (has_more > 0) {
    ...
    LOG(log_level::info) << "Data to write " << cur_offset_ << " " << data_to_write.size() << " " << has_more << " " << cur_partition_;
    blocks_[block_id()]->send_command(args);

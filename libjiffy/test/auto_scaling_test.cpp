...
TEST_CASE("msg_queue_auto_scale_test", "[directory_service][storage_server][management_server]") {
  ...
  auto block_names = test_utils::init_block_names(2, STORAGE_SERVICE_PORT, STORAGE_MANAGEMENT_PORT);
  alloc->add_blocks(block_names);
  auto blocks = test_utils::init_hash_table_blocks(block_names, 15000);
  ...
  REQUIRE_NOTHROW(t->create("/sandbox/scale_up.txt", "msgqueue", "/tmp", 1, 1, 0, perms::all(), {"0"}, {"regular"}, {}));
  ...
  for (std::size_t i = 0; i < 500; i++) {
    REQUIRE(std::dynamic_pointer_cast<msg_queue_partition>(blocks[0]->impl())->read(std::to_string(i)) == std::to_string(i));
  }
/*

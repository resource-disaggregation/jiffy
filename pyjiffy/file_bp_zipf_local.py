...
class FileBenchmark:
	...
	def wait(self):
		...
		return [throughput, latency_write / self.num_clients, latency_read / self.num_clients, float(hit * 100 / access), access]
...
class ReadBenchmark(FileBenchmark):
	...
	def single_thread_action(self, thread_index):
		...
		for j in range(self.num_ops_):
			...
			cache_hit += resp[2]
			reading_time = resp[3]
			prefetch_time = resp[4]
			hit_handling_time = resp[5]
		print("reading_time= ",reading_time,"prefetch_time= ",prefetch_time,"hit_handling_time= ",hit_handling_time)
...
def run_benchmark():
	...
	file_name = './benchmark/file_bp_zipf_local.txt'
		...
		while count <= 1:
			...
			for cache_block_size in range(100, 501, 400): 
				for prefetch_size in range(100, 201, 50):
					...
					print("\tAccess_time: ", result[4], file=data)
					print("\n", file=data)
					loading += 1

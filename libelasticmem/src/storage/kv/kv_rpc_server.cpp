...
std::shared_ptr<TThreadedServer> kv_rpc_server::create(std::vector<std::shared_ptr<kv_block>> &blocks,
                                                       ...
                                                       int port) {
  std::shared_ptr<kv_rpc_serviceIfFactory> clone_factory(new kv_rpc_service_factory(blocks, sub_maps));
  std::shared_ptr<kv_rpc_serviceProcessorFactory> proc_factory(new kv_rpc_serviceProcessorFactory(clone_factory));
